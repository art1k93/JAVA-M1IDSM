...
/**
 * The LibTicket class provides the possibility to create a library ticket for
 * reader.
 *
 */
public class LibTicket {
	...
	LibTicket(final Reader u, final Book ub, final Date tu, final Date tr) {
		System.out.println("class LibTicket: constructor is initialized.");
		user = u;
		userBook = ub;
		termUse = tu;
		termReturn = tr;
	...
	public void getObjInfo() {
		System.out.printf(
				"Library ticket info:\n\tWho took a book: %s\n\t" + "What book was taken: %s (author: %s)\n\t"
						+ "Term of use: %s\n\tTerm of return: %s\n\n",
				getUser().getName(), getUserBook().getBookName(), getUserBook().getAuthor(), getTermUse(),
				getTermReturn());
	}
}
